...
Vue.component('line-item', {
    ...
    methods: {
        ...
        customRemove() {
            let itemsRef = this.$root.items;
            for(let i = 0; i < itemsRef.length; ++i) {
                itemsRef[i].id = i;
            }

            this.$root.items.splice(this.item.id,1);
    ...
    '<v-btn color="error" v-on:click="customRemove">Delete</v-btn></v-list>'
});
...
new Vue({
    ...
    data: {
        items: [
            {id: 0, name: 'Furbies', quantity: 20000, isFavorite: true},
            {id: 1, name: 'Tonnes of AAA batteries', quantity: 100, isFavorite: true},
            {id: 2, name: 'Furby Sized Rail Guns', quantity: 200000, isFavorite: false},
            {id: 3, name: 'Ai-chips', quantity: 200000, isFavorite: false}
        ],
